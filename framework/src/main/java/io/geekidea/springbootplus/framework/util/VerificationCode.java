...
public class VerificationCode {
     ...
     * 验证码数组
     **/
    private String codes = digits + letters;
    /**
     * 生成的验证码的个数
     **/
     ...
     * 获取随机字符
     */
    private char randomChar(boolean digitsOnly) {
        if (digitsOnly) {
    ...
    public String getCodeOnly() {
        return getCodeOnly(this.codeNum);
    }
     ...
     * @return
     */
    public String getCodeOnly(int length) {
        return getCodeOnly(length, false);
    }

    /**
     * 获得指定长度的随机验证码
     * @param length
     * @param digitsOnly 是否只获取数字类型
     * @return
     */
    public String getCodeOnly(int length, boolean digitsOnly) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < length; i++) {
            //随机生成字符添加到StringBuilder里面
            sb.append(randomChar(digitsOnly));
        }
        this.text = sb.toString();
        return this.text;
    }
