...
//                .build();
        ...
        return result(apiCode.getCode(), StringUtils.isBlank(message) ? apiCode.getMessage() : message, data);
    }

    public static <T> ApiResult<T> result(Integer apiCode, String message, T data) {
        boolean success = false;
        if (ApiCode.SUCCESS.getCode() == apiCode){
            success = true;
        }
